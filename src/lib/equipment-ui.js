...
function allowedSlots(item) {
  if (item.category==='weapon') return EQUIPMENT_SLOTS.filter(s=>s.key==='main_hand'||(s.key==='off_hand'&&Number(item.hands||0)!==2));
  if (item.category==='armor') return EQUIPMENT_SLOTS.filter(s=>s.key==='body');
  if (item.category==='accessory') return EQUIPMENT_SLOTS.filter(s=>s.key==='accessory_1'||s.key==='accessory_2');
  return [];
}

function weaponSummary(item) {
  if (item.category!=='weapon') return '';
  const p=WEAPON_PROFILES[item.weapon_profile]||WEAPON_PROFILES.standard;
  ...
  return `${p.damageDiceCount}d${p.damageDie} + Mod. ${damageAttr} • ${p.paCost} PA • ${attr} + ${skill} • ${item.hands} mão(s)`;
}
...
function createFormHtml(master=false) {
  return `<form class="grid equipment-create-form">
    ...
    <div class="combat-config-box equipment-weapon-fields"><h3>Perfil de arma</h3>
      ...
      <div class="field-row"><label>Alcance<select name="weaponRange"><option value="melee">Corpo a corpo</option><option value="ranged">À distância</option></select></label><label>Mãos (apenas perfil Padrão)<select name="hands"><option value="1">1 mão</option><option value="2">2 mãos</option></select></label></div>
...
function effectFormHtml(item, master=false, conditions=[], esc=(v)=>String(v??'')) {
  ...
  return `<form class="grid equipment-effect-form" data-item-id="${item.id}">
    ...
    <div class="field-row-3"><label>PA<input name="pa" type="number" min="0" max="7" value="1" /></label><label>EA<input name="ea" type="number" min="0" value="0" /></label><label>Cargas por uso<input name="chargesCost" type="number" min="0" value="0" /></label></div>
...
function itemCardHtml(item, master, esc, conditions=[]) {
  ...
  return `<div class="list-item equipment-card" data-equipment-card="${item.id}">
    <div class="equipment-card-head">${image}<div><div class="btn-row"><div class="title">${esc(item.name)}</div>${statusPill(item.status)}${item.is_cursed?`<span class="pill bad">${esc(item.grade)}</span>`:'<span class="pill">Comum</span>'}${item.equipped?`<span class="pill good">${esc(equipmentSlotName(item.equip_slot))}</span>`:''}</div><div class="meta">${esc(equipmentCategoryName(item.category))}${item.subtype?` • ${esc(item.subtype)}`:''}${item.is_cursed?` • VP ${used}/${budget}${item.vp_limit_override?' + exceção do Mestre':''}`:''}</div></div></div>
    ${item.category==='weapon'?`<div class="notice" style="margin-top:8px"><strong>Ataque básico:</strong> ${esc(weaponSummary(item))}. Pode receber +1 EA no uso para conduzir Energia Amaldiçoada e ficar elegível a Kokusen.</div>`:''}
...
function bindCreateForm(root, character, master, ctx, rerender) {
  ...
  const refresh=()=>{
    weaponBox.style.display=category.value==='weapon'?'grid':'none';
    ...
    form.elements.hands.disabled=p.handsMin===p.handsMax;
    if(p.handsMin===p.handsMax) form.elements.hands.value=String(p.handsMin);
  ...
  form.onsubmit=async e=>{
    ...
    const payload={
      ...
      hands:cat==='weapon'?Number(f.get('hands')||1):0,
...
function bindItemActions(root, items, master, ctx, rerender) {
  ...
  root.querySelectorAll('.equipment-effect-form').forEach(form=>{
    ...
    const applyTypeDefaults=()=>{const type=form.elements.effectType.value;if(type==='passive'){form.elements.pa.value=0;form.elements.requiresAttack.checked=false;}else if(type==='attack'){if(Number(form.elements.pa.value||0)===0)form.elements.pa.value=1;form.elements.requiresAttack.checked=true;}refreshVp();};
...
export async function renderPlayerEquipmentPage(ctx) {
  ...
  root.innerHTML=`${pageHeader('Armas, amuletos e ferramentas','Inventário')}
    <div class="notice">Itens comuns podem ser usados imediatamente. Ferramentas amaldiçoadas criadas pelo jogador ficam pendentes até o Mestre aprovar seus efeitos e grau.</div>
    <div style="height:14px"></div><section class="grid grid-2"><div class="card"><h2>Novo equipamento</h2>${createFormHtml(false)}</div><div class="card"><h2>Regras rápidas</h2><p><strong>Armas:</strong> o perfil físico define o dano básico sem consumir VP.</p><p><strong>Ferramentas amaldiçoadas:</strong> Grau 4 = 2 VP, Grau 3 = 4 VP, Grau 2 = 6 VP, Grau 1 = 9 VP, Grau Especial = 12 VP base.</p><p><strong>Equipar:</strong> somente itens aprovados fornecem acesso a ataques e efeitos em combate.</p></div></section>
    <div style="height:14px"></div><section class="card"><h2>Meus equipamentos</h2><div class="list">${items.length?items.map(i=>itemCardHtml(i,false,esc,state.conditions)).join(''):'<p class="muted">Nenhum equipamento.</p>'}</div></section>`;
...
export async function renderMasterEquipmentManager(root, character, ctx, rerender) {
  ...
  root.innerHTML=`<div class="master-zone"><h2>Equipamentos da entidade</h2><div class="grid grid-2"><div class="card"><h3>Novo equipamento</h3>${createFormHtml(true)}</div><div class="card"><h3>Regras</h3><p class="muted">Equipamentos criados pelo Mestre já nascem aprovados. O ataque físico da arma não consome VP; efeitos sobrenaturais consomem o orçamento do grau.</p></div></div><div style="height:10px"></div><div class="list">${items.length?items.map(i=>itemCardHtml(i,true,esc,state.conditions)).join(''):'<p class="muted">Nenhum equipamento.</p>'}</div></div>`;
