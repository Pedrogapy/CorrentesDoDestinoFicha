...
async function executeEquipment(item, actorId, encounterId, targetId) {
...
export async function renderPlayerCombatPageV2(ctx) {
  ...
  const attackEquipment=equipment.filter(i=>equipmentDefaults(i).enabled);
  root.innerHTML=`${pageHeader(`Rodada ${active.round}`,'Combate')}
    ...
    <div class="card"><h2>Ataques de equipamento</h2><div class="list">${attackEquipment.map(i=>{const c=equipmentDefaults(i);return `<div class="list-item"><div class="title">${esc(i.name)}</div><div class="meta">${c.paCost} PA • ${c.eaCost} EA • ${c.damageDiceCount}d${c.damageDie}</div><label style="margin-top:8px">Alvo<select data-equipment-target="${i.id}">${targetOpts}</select></label><button class="btn" data-use-equipment="${i.id}" style="margin-top:8px">Atacar com equipamento</button></div>`}).join('')||'<p class="muted">Nenhum equipamento com ataque configurado.</p>'}</div></div></section>
