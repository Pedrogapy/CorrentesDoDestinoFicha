...
import { ATTRIBUTES, SKILLS, ATTRIBUTE_BY_KEY, SKILL_BY_KEY, characterDerived } from './system.js';
...
async function executeEquipment(item, actorId, encounterId, targetId, reinforce=false) {
  ...
  return api.createCombatAttack({
    encounterId, attackerCharacterId:actorId, targetCharacterId:targetId, label:item.name,
    sourceType:'equipment', sourceId:item.id, attackAttributeKey:c.attackAttributeKey, attackSkillKey:c.attackSkillKey,
    paCost:c.paCost, eaCost:c.eaCost+extraEa, usesCursedEnergy:c.usesCursedEnergy||extraEa>0, forcedCritical:c.forcedCritical,
    criticalThreshold:c.criticalThreshold, damageDiceCount:c.damageDiceCount, damageDie:c.damageDie,
    damageFlatAttributeKey:c.damageFlatAttributeKey, conditionKey:c.conditionKey,
  });
}
...
export async function renderPlayerCombatPageV2(ctx) {
  ...
  const attackEquipment=usableEquipment.filter(i=>equipmentDefaults(i).enabled);
  ...
  const passiveEquipment=usableEquipment.flatMap(i=>(Array.isArray(i.effects)?i.effects:[]).filter(e=>e.type==='passive').map(effect=>({item:i,effect})));
...
export async function renderMasterCombatPageV2(ctx) {
  ...
  const approvedAbilities=abilities.filter(a=>a.status==='approved'); const usableEquipment=equipment.filter(i=>i.status==='approved'&&(i.equipped||i.category==='consumable')); const attackEquipment=usableEquipment.filter(i=>equipmentDefaults(i).enabled); const effectEquipment=usableEquipment.flatMap(i=>(Array.isArray(i.effects)?i.effects:[]).filter(e=>['active','reaction','attack'].includes(e.type)).map(effect=>({item:i,effect}))); const passiveEquipment=usableEquipment.flatMap(i=>(Array.isArray(i.effects)?i.effects:[]).filter(e=>e.type==='passive').map(effect=>({item:i,effect})));
  ...
  root.innerHTML=`${pageHeader(`Rodada ${active.round}`,'Combate do Mestre','<span class="pill bad">Rolagens do Mestre ficam ocultas para jogadores</span>')}
    ...
    ${actor?`<div style="height:14px"></div><section class="grid grid-2"><div class="card"><h2>Habilidades de ${esc(getName(actor))}</h2><div class="list">${approvedAbilities.map(a=>{const c=sourceDefaults(a);return `<div class="list-item"><div class="title">${esc(a.name)}</div><div class="meta">${c.paCost} PA • ${c.eaCost} EA</div><label>Alvo<select data-master-ability-target="${a.id}">${targetOpts}<option value="${actor.id}">${esc(getName(actor))}</option></select></label><button class="btn bad" data-master-use-ability="${a.id}" style="margin-top:8px">Usar</button></div>`}).join('')||'<p class="muted">Nenhuma habilidade aprovada.</p>'}</div></div><div class="card"><h2>Equipamentos equipados</h2><div class="list">${attackEquipment.map(i=>{const c=equipmentDefaults(i);return `<div class="list-item"><div class="title">${esc(i.name)}</div><label>Alvo<select data-master-equipment-target="${i.id}">${targetOpts}</select></label>${!c.usesCursedEnergy?`<label style="display:flex;align-items:center;gap:7px;margin-top:8px"><input type="checkbox" data-master-equipment-reinforce="${i.id}" style="width:auto" /> Conduzir +1 EA</label>`:''}<button class="btn bad" data-master-use-equipment="${i.id}" style="margin-top:8px">Atacar</button></div>`}).join('')||'<p class="muted">Nenhuma arma equipada.</p>'}${effectEquipment.map(({item,effect})=>{const c=equipmentEffectCombatDefaults(effect);return `<div class="list-item"><div class="title">${esc(item.name)} • ${esc(effect.name)}</div><div class="meta">${c.paCost} PA • ${c.eaCost} EA${c.chargesCost?` • ${c.chargesCost} carga(s)`:''}</div><label>Alvo<select data-master-equipment-effect-target="${esc(effect.id)}" data-item-id="${item.id}">${targetOpts}<option value="${actor.id}">${esc(getName(actor))}</option></select></label><button class="btn bad" data-master-use-equipment-effect="${esc(effect.id)}" data-item-id="${item.id}" style="margin-top:8px">Usar efeito</button></div>`}).join('')}${passiveEquipment.map(({item,effect})=>`<div class="list-item"><div class="title">${esc(item.name)} • ${esc(effect.name)}</div><span class="pill good">Passivo equipado</span><div class="body" style="margin-top:6px">${esc(effect.mechanics||effect.description||'')}</div></div>`).join('')}</div></div></section>`:''}
