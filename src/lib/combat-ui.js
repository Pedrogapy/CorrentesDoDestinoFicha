...
function participantCard(p, ctx, editable=false, activeParticipantId=null) {
  const { esc, state } = ctx;
...
async function executeEquipmentEffect(item, effect, actorId, encounterId, targetId) {
  ...
  if (c.requiresAttack) {
    result=await api.createCombatAttack({
      ...
      sourceType:effect.type==='reaction'?'equipment_reaction':'equipment', sourceId:item.id, attackAttributeKey:c.attackAttributeKey, attackSkillKey:c.attackSkillKey,
    ...
    result=await api.useCombatEffect({
      ...
      paCost:c.paCost, eaCost:c.eaCost, damageDiceCount:c.damageDiceCount, damageDie:c.damageDie,
      damageFlatAttributeKey:c.damageFlatAttributeKey, conditionKey:c.conditionKey, isReaction:effect.type==='reaction',
    });
  }
...
export async function renderPlayerCombatPageV2(ctx) {
  ...
  const turnBanner=!mine
    ...
    : isMyTurn
      ? `<section class="card"><div class="eyebrow">TURNO ATIVO</div><h2 style="margin-bottom:6px">Sua vez, ${esc(myName)}!</h2><div class="notice">O Mestre iniciou seu turno. Suas ações de turno estão liberadas. Quando terminar, você ou o Mestre podem encerrar o turno sem gastar PA ou EA.</div></section>`
      : activeParticipant
        ? `<section class="card"><div class="eyebrow">AGUARDANDO TURNO</div><h2 style="margin-bottom:6px">Agora é a vez de ${esc(activeName)}.</h2><div class="notice">Suas ações de turno estão bloqueadas. Reações oferecidas pelo combate, como Esquivar, Defender, Reforçar, Resistir e Contra-atacar, continuam disponíveis quando forem acionadas.</div></section>`
        : `<section class="card"><div class="eyebrow">AGUARDANDO O MESTRE</div><h2 style="margin-bottom:6px">Nenhum turno foi iniciado.</h2><div class="notice">Aguarde o Mestre escolher quem vai agir. Quando ele iniciar seu turno, esta tela será liberada automaticamente.</div></section>`;
  root.innerHTML=`${pageHeader(`Rodada ${active.round}`,'Combate')}
    ${turnBanner}<div style="height:14px"></div>
    <section class="grid grid-2"><div class="card"><h2>${esc(active.name)}</h2>${mine?participantCard(mine,ctx,false,active.active_participant_id):'<p class="muted">Seu personagem ainda não foi adicionado.</p>'}</div>
    <div class="card"><h2>Golpe corpo a corpo</h2>${mine&&targetOpts&&isMyTurn?`<form id="basic-attack" class="grid"><label>Alvo<select name="target">${targetOpts}</select></label><label style="display:flex;align-items:center;gap:7px"><input name="cursed" type="checkbox" style="width:auto" /> Reforçar o golpe com Energia Amaldiçoada • +1 EA • permite Kokusen em 20 natural</label>${modeFields('')}<div class="notice">1 PA • Força + Lutar • dano 1d6 + Mod. Força</div><button class="btn primary">Atacar</button></form>`:mine&&targetOpts?'<div class="notice">Aguardando o Mestre iniciar seu turno.</div>':'<p class="muted">É preciso estar no combate e possuir um alvo.</p>'}</div></section>
    <div style="height:14px"></div>
    <section class="grid grid-2"><div class="card"><h2>Habilidades aprovadas</h2><div class="list">${approvedAbilities.map(a=>{const c=sourceDefaults(a);return `<div class="list-item"><div class="title">${esc(a.name)}</div><div class="meta">${c.paCost} PA • ${c.eaCost} EA • ${c.requiresAttack?`${esc(ATTRIBUTE_BY_KEY[c.attackAttributeKey]?.name||c.attackAttributeKey)} + ${esc(SKILL_BY_KEY[c.attackSkillKey]?.name||c.attackSkillKey)}`:'efeito direto'}</div><div class="body">${esc(a.mechanics||a.description||'')}</div><label style="margin-top:8px">Alvo<select data-ability-target="${a.id}" ${isMyTurn?'':'disabled'}>${targetOpts}<option value="${state.character.id}">Eu mesmo</option></select></label><button class="btn primary" data-use-ability="${a.id}" style="margin-top:8px" ${isMyTurn?'':'disabled'}>Usar habilidade</button></div>`}).join('')||'<p class="muted">Nenhuma habilidade aprovada.</p>'}</div>${!isMyTurn&&mine?'<div class="notice" style="margin-top:10px">Habilidades de turno ficam disponíveis quando o Mestre iniciar sua vez.</div>':''}</div>
    <div class="card"><h2>Equipamentos equipados</h2><div class="list">${attackEquipment.map(i=>{const c=equipmentDefaults(i);const base=weaponDamageProfile(i.weapon_profile||'standard',false);const canTwo=i.weapon_profile==='standard'&&i.equip_slot==='main_hand'&&offHandFree;return `<div class="list-item"><div class="title">${esc(i.name)}</div><div class="meta">${base.paCost} PA • ${base.damageDiceCount}d${base.damageDie}${canTwo?' • 2 mãos: 1d10':''}</div><label style="margin-top:8px">Alvo<select data-equipment-target="${i.id}" ${isMyTurn?'':'disabled'}>${targetOpts}</select></label>${canTwo?`<label style="display:flex;align-items:center;gap:7px;margin-top:8px"><input type="checkbox" data-equipment-two-hands="${i.id}" style="width:auto" ${isMyTurn?'':'disabled'} /> Empunhar com duas mãos neste ataque • dano 1d10</label>`:''}${!c.usesCursedEnergy?`<label style="display:flex;align-items:center;gap:7px;margin-top:8px"><input type="checkbox" data-equipment-reinforce="${i.id}" style="width:auto" ${isMyTurn?'':'disabled'} /> Conduzir +1 EA neste golpe • permite Kokusen em 20 natural</label>`:''}<button class="btn" data-use-equipment="${i.id}" style="margin-top:8px" ${isMyTurn?'':'disabled'}>Atacar com equipamento</button></div>`}).join('')||'<p class="muted">Nenhuma arma equipada.</p>'}${effectEquipment.map(({item,effect})=>{const c=equipmentEffectCombatDefaults(effect);return `<div class="list-item"><div class="title">${esc(item.name)} • ${esc(effect.name)}</div><div class="meta">${c.paCost} PA • ${c.eaCost} EA${c.chargesCost?` • ${c.chargesCost} carga(s)`:''}</div><div class="body">${esc(effect.mechanics||effect.description||'')}</div><label style="margin-top:8px">Alvo<select data-equipment-effect-target="${esc(effect.id)}" data-item-id="${item.id}" ${isMyTurn?'':'disabled'}>${targetOpts}<option value="${state.character.id}">Eu mesmo</option></select></label><button class="btn primary" data-use-equipment-effect="${esc(effect.id)}" data-item-id="${item.id}" style="margin-top:8px" ${isMyTurn?'':'disabled'}>Usar efeito</button></div>`}).join('')}${passiveEquipment.map(({item,effect})=>`<div class="list-item"><div class="title">${esc(item.name)} • ${esc(effect.name)}</div><span class="pill good">Passivo equipado</span><div class="body" style="margin-top:6px">${esc(effect.mechanics||effect.description||'')}</div></div>`).join('')}</div>${!isMyTurn&&mine?'<div class="notice" style="margin-top:10px">Ataques e efeitos ativos ficam bloqueados fora do seu turno.</div>':''}</div></section>
...
export async function renderMasterCombatPageV2(ctx) {
  ...
  const turnStatus=activeParticipant
    ? `<div class="eyebrow">TURNO ATIVO</div><h2 style="margin:0 0 6px">Vez de ${esc(actorName)}</h2><div class="notice">Somente ${esc(actorName)} pode iniciar ações de turno. Reações dos outros participantes continuam funcionando normalmente. O turno pode ser encerrado pelo Mestre ou pelo próprio jogador.</div>`
    : `<div class="eyebrow">AGUARDANDO INÍCIO DE TURNO</div><h2 style="margin:0 0 6px">Escolha quem vai agir</h2><div class="notice">Clique em <strong>Iniciar turno</strong> no participante desejado. Enquanto nenhum turno estiver ativo, ataques, habilidades e testes de combate ficam bloqueados.</div>`;
  root.innerHTML=`${pageHeader(`Rodada ${active.round}`,'Combate do Mestre','<span class="pill bad">Rolagens do Mestre ficam ocultas para jogadores</span>')}
    <section class="card combat-master-controls"><div class="btn-row"><div><strong>${esc(active.name)}</strong><div class="muted small">${activeParticipant?`Turno iniciado para ${esc(actorName)}.`:'Nenhum turno ativo.'}</div>${lastUndo?`<div class="muted small" style="margin-top:4px">Última ação desfazível: <strong>${esc(lastUndo.label)}</strong></div>`:'<div class="muted small" style="margin-top:4px">Ainda não há ação para desfazer.</div>'}</div><div class="btn-row"><button class="btn warn" id="undo-combat" ${lastUndo?'':'disabled'}>Desfazer última ação</button><button class="btn bad" id="end-encounter">Encerrar combate</button></div></div><div class="notice" style="margin-top:10px">Iniciar e encerrar turno também são ações desfazíveis. Encerrar turno não gasta PA nem EA; apenas aplica os efeitos normais de fim de turno.</div></section>
    <div style="height:14px"></div><section class="card">${turnStatus}</section>
    <div style="height:14px"></div>
    <section class="grid grid-2"><div class="card"><h2>Participantes</h2><div class="list">${participants.map(p=>participantCard(p,ctx,true,active.active_participant_id)).join('')||'<p class="muted">Vazio.</p>'}</div><h3>Adicionar</h3><div class="btn-row">${state.masterCharacters.filter(c=>!inCombat.has(c.id)).map(c=>`<button class="btn" data-add-combat="${c.id}">${esc(getName(c))}</button>`).join('')}</div></div>
    <div class="card"><h2>Ações do turno</h2>${actor?`<div class="list-item"><div class="title">Entidade em turno: ${esc(actorName)}</div><div class="meta">O Mestre pode controlar esta entidade enquanto o turno dela estiver ativo.</div></div><form id="master-basic" class="grid" style="margin-top:10px"><h3>Golpe corpo a corpo</h3><label>Alvo<select name="target">${targetOpts}</select></label><label style="display:flex;align-items:center;gap:7px"><input name="cursed" type="checkbox" style="width:auto" /> Reforçar com 1 EA</label><button class="btn bad">Atacar em segredo</button></form><hr style="border-color:#333"><form id="master-skill" class="grid"><h3>Teste secreto</h3><label>Perícia<select name="skill">${optionList(SKILLS)}</select></label>${modeFields('')}<button class="btn bad">Rolar em segredo</button></form>`:'<div class="notice">Nenhuma entidade pode realizar ação de turno até você iniciar o turno de um participante.</div>'}</div></section>
    ${actor?`<div style="height:14px"></div><section class="grid grid-2"><div class="card"><h2>Habilidades de ${esc(actorName)}</h2><div class="list">${approvedAbilities.map(a=>{const c=sourceDefaults(a);return `<div class="list-item"><div class="title">${esc(a.name)}</div><div class="meta">${c.paCost} PA • ${c.eaCost} EA</div><label>Alvo<select data-master-ability-target="${a.id}">${targetOpts}<option value="${actor.id}">${esc(actorName)}</option></select></label><button class="btn bad" data-master-use-ability="${a.id}" style="margin-top:8px">Usar</button></div>`}).join('')||'<p class="muted">Nenhuma habilidade aprovada.</p>'}</div></div><div class="card"><h2>Equipamentos equipados</h2><div class="list">${attackEquipment.map(i=>{const c=equipmentDefaults(i);const base=weaponDamageProfile(i.weapon_profile||'standard',false);const canTwo=i.weapon_profile==='standard'&&i.equip_slot==='main_hand'&&offHandFree;return `<div class="list-item"><div class="title">${esc(i.name)}</div><div class="meta">${base.paCost} PA • ${base.damageDiceCount}d${base.damageDie}${canTwo?' • 2 mãos: 1d10':''}</div><label>Alvo<select data-master-equipment-target="${i.id}">${targetOpts}</select></label>${canTwo?`<label style="display:flex;align-items:center;gap:7px;margin-top:8px"><input type="checkbox" data-master-equipment-two-hands="${i.id}" style="width:auto" /> Empunhar com duas mãos neste ataque</label>`:''}${!c.usesCursedEnergy?`<label style="display:flex;align-items:center;gap:7px;margin-top:8px"><input type="checkbox" data-master-equipment-reinforce="${i.id}" style="width:auto" /> Conduzir +1 EA</label>`:''}<button class="btn bad" data-master-use-equipment="${i.id}" style="margin-top:8px">Atacar</button></div>`}).join('')||'<p class="muted">Nenhuma arma equipada.</p>'}${effectEquipment.map(({item,effect})=>{const c=equipmentEffectCombatDefaults(effect);return `<div class="list-item"><div class="title">${esc(item.name)} • ${esc(effect.name)}</div><div class="meta">${c.paCost} PA • ${c.eaCost} EA${c.chargesCost?` • ${c.chargesCost} carga(s)`:''}</div><label>Alvo<select data-master-equipment-effect-target="${esc(effect.id)}" data-item-id="${item.id}">${targetOpts}<option value="${actor.id}">${esc(actorName)}</option></select></label><button class="btn bad" data-master-use-equipment-effect="${esc(effect.id)}" data-item-id="${item.id}" style="margin-top:8px">Usar efeito</button></div>`}).join('')}${passiveEquipment.map(({item,effect})=>`<div class="list-item"><div class="title">${esc(item.name)} • ${esc(effect.name)}</div><span class="pill good">Passivo equipado</span><div class="body" style="margin-top:6px">${esc(effect.mechanics||effect.description||'')}</div></div>`).join('')}</div></div></section>`:''}
